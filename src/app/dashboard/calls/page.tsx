...
import { Search, PhoneCall, ChevronDown, ChevronUp, Download, FileText, X } from "lucide-react";
...
function CallsContent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      const [custData, callsData] = await Promise.all([
  ...
  const groupedData = useMemo(() => {
    const filteredCalls = calls.filter(call => {
      const custName = customers[call.customerId]?.name?.toLowerCase() || "";
      if (searchQuery && !custName.includes(searchQuery.toLowerCase())) {
        return false;
      }
      return true;
    });

    const group: Record<string, CallData[]> = {};

    filteredCalls.forEach(call => {
      const custId = call.customerId || "unknown";
      if (!group[custId]) group[custId] = [];
      group[custId].push(call);
    });

    const result = Object.entries(group).map(([custId, custCalls]) => {
      const overallLatestTime = custCalls.length > 0 ? new Date(custCalls[0].startTime).getTime() : 0;
      return {
        customerId: custId,
        customer: customers[custId],
        calls: custCalls,
        totalCalls: custCalls.length,
    ...
    return result;
  }, [calls, customers, searchQuery, expandedCustomers]);

  const modalCall = useMemo(() => {
    if (!modalCallSid) return null;
    const call = calls.find(c => c.callSid === modalCallSid);
    if (!call) return null;
    const customerName = customers[call.customerId]?.name || "Unknown Customer";
    const duration = calculateDuration(call.startTime, call.endTime) || call.duration;
    let recordingUrl = call.recordingUrl;
    if (!recordingUrl && call.status?.includes('| Recording: ')) {
      recordingUrl = call.status.split('| Recording: ')[1];
    }
    return { ...call, customerName, calculatedDuration: duration, activeRecordingUrl: recordingUrl };
  }, [modalCallSid, calls, customers]);
    ...
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6 pb-20 relative">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Review call histories and transcripts grouped by customer</p>
        </div>
      ...
      <div className="space-y-6 mt-6">
        {groupedData.length === 0 ? (
          <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center shadow-sm">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">No calls found.</h3>
            <p className="text-gray-500">Make your first call to see it here.</p>
          </div>
        ) : (
                ...
                {isExpanded && (
                  <div className="border-t border-gray-200 bg-gray-50 p-4 md:p-6 space-y-4">
                    {group.calls.map((call) => {
                      ...
                      const formattedDate = callDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
                      const formattedTime = callDate.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
                      const duration = calculateDuration(call.startTime, call.endTime) || call.duration;

                      return (
                        <div key={call.callSid} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                          <div 
                            onClick={(e) => openModal(call.callSid, e)}
                            ...
                            <div className="flex items-center gap-3">
                              <div>
                                ...
                                <p className="text-xs text-gray-500 mt-0.5 font-mono">
                                  SID: {call.callSid ? call.callSid.substring(0, 8) : 'N/A'}... {duration && `• Duration: ${duration}`}
                                </p>
                              </div>
                            </div>
      ...
      {modalCall && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={closeModal}>
          ...
          >
            ...
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-gray-100 flex flex-col items-start bg-gray-50 sticky top-0 z-10 pr-16">
              <h3 className="font-bold text-gray-900 text-lg flex items-center gap-2">
                Call Detail
              </h3>
              <p className="text-sm text-gray-500 mt-1">
                {modalCall.customerName} • {new Date(modalCall.startTime).toLocaleDateString()} {new Date(modalCall.startTime).toLocaleTimeString()}
              </p>
              <p className="text-xs text-gray-400 mt-0.5 font-mono">
                SID: {modalCall.callSid ? modalCall.callSid.substring(0, 12) : 'N/A'}... {modalCall.calculatedDuration && `• Duration: ${modalCall.calculatedDuration}`}
              </p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto flex flex-col">
              ...
              {modalCall.activeRecordingUrl && (
                <div className="px-6 py-4 border-b border-gray-100 bg-[#FAFAFA] shrink-0 sticky top-0 z-10">
                  ...
                  <audio controls src={`http://localhost:8080/api/conversations/${modalCall.callSid}/recording/download`} className="w-full h-10" />
                </div>
              )}
